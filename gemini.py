...
modelDynamo = gemini.GenerativeModel(
...
modelSeeker = gemini.GenerativeModel(
    model_name="gemini-2.0-flash",
    safety_settings=[
        {
            "category": "HARM_CATEGORY_HARASSMENT",
...
modelShaman = gemini.GenerativeModel(
    model_name="gemini-2.0-flash",
    safety_settings=[
        {
            "category": "HARM_CATEGORY_HARASSMENT",
...
modelRadar = gemini.GenerativeModel(
    model_name="gemini-2.0-flash",
    safety_settings=[
        {
            "category": "HARM_CATEGORY_HARASSMENT",
...
modelAce = gemini.GenerativeModel(
    model_name="gemini-2.0-flash",
    safety_settings=[
        {
            "category": "HARM_CATEGORY_HARASSMENT",
...
projectModelAce = gemini.GenerativeModel(
    model_name="gemini-2.0-flash",
    safety_settings=[
        {
            "category": "HARM_CATEGORY_HARASSMENT",
...
def config_dynamo(icebreaker, nickname, history, instructions):
    resp = dynamo.send_message(
        prompts.prompt_corpus().get_chat_config(
            icebreaker, nickname, history, mode="dynamo", mask=instructions
        )
    )
    return resp
...
def config_shaman(icebreaker, nickname, history):
    resp = shaman.send_message(
        prompts.prompt_corpus().get_chat_config(
            icebreaker, nickname, history, mode="shaman"
        )
    )
    return resp
...
def config_seeker(icebreaker, nickname, history):
    resp = seeker.send_message(
        prompts.prompt_corpus().get_chat_config(
            icebreaker, nickname, history, mode="seeker"
        )
    )
    return resp
...
def config_context_seeker(nickname, corpus):
    resp = context_seeker.send_message(
        prompts.prompt_corpus().get_wiki_context(nickname, corpus)
    )
    return resp


def config_ace(nickname):
    resp = ace.send_message(
        prompts.prompt_corpus().get_chat_config(
            "How can I help you?", nickname, history="", mode="ace"
        )
    )
    return resp
...
def config_project_ace(
    nickname, project_title, project_details, project_tasks, context, catchup
):
    global project_ace
    project_ace = reset_project_ace_history()
    config = prompts.prompt_corpus().get_ace_project_config(
        nickname, project_title, project_details, project_tasks, context, catchup
    )
...
def chat_project_ace(message, image=None):
    if image:
        return (
            project_ace.send_message([message, PIL.Image.open(image)]),
            project_ace.history[-1],
        )
    return project_ace.send_message(message), project_ace.history[-1]


def chat_radar(question, user_response):
    prompt = prompts.prompt_corpus().get_radar_prompt(question, user_response)
    return radar.send_message(prompt)
...
def config_philosopher(nickname, philosopher_name, icebreaker):
    resp = philosopher.send_message(
        prompts.prompt_corpus().get_philosopher_mask(
            nickname, philosopher_name, icebreaker
        )
    )
    return resp
