...
class curate_web:
    ...
    def no_history_response(self, opted):
        return (
            "Once you start interacting with Alcove, results will be reflected. Get started!"
            if opted
            else "You are not using Personalization (Turn it on). Once you Opt-In, results will be reflected."
        )

    def get_tag_tooltip(self, user_class: str) -> str:
        tooltips = {
            "Shaman": """Relax and engage in mindfulness activities,
                whether it be guided meditation, seeking therapy
                or doing things your way.
                """,
            "Ace": """Utilise your time by planning your routine,
                organizing your schedule or using various tools
                to enhance efficiency. Or, just get some advice.
                """,
            "Challenger": """Brainstorm over tricky riddles, challenging
                puzzles or go against an unbeatable opponent
                in various strategy games.
                """,
            "Generalist": """Find a new hobby for yourself, and learn 
                   how to approach it. Or, discover new ways
                   to enrich those you already enjoy!
                """,
            "Dynamo": """Engage in quick and dynamic conversations with 
                Dynamo, your AI-chat companion on Alcove""",
            "Seeker": """Debate on various philosophies and paradoxes, or
                just discover something you'd like to learn or know
                about.
                """,
            "Wanderer": """Plan your next vacation or discover various points
    ...
    def yesterday(self):
        ...
        return (
            str(yesterday.strftime("%A"))
            + ", "
            + str(yesterday.strftime("%d"))
            + " "
            + str(yesterday.strftime("%B"))
            + " "
            + str(yesterday.strftime("%Y"))
        )
    ...
    def get_radar_question(self, id):
        qset = libs.get_radar_question_set()
        return libs.get_radar_question_set()[id]
