...
def home():
    ...
    if session:
        ...
        if is_a_first_visit:
            ...
            recommendations = [
                tag.capitalize()
                for tag in gemini.fit_prompt(
                    prompt_handler.get_sentiment_corpus(tags, True)
                ).split()
            ]
        else:
...
def survey_submit():
    ...
    for cookie in request.cookies:
        if "first_visit" in cookie:
            ...
            user_class = gemini.fit_prompt(
                prompt_handler.get_sentiment_corpus(
                    tags=home_engine.build_tags(session["nickname"])
                )
            )
            session["user_class"] = user_class.capitalize()
            mongo.db.user_classes.insert_one(
                {"nickname": session["nickname"], "class": user_class}
            )
    return response
...
def dynamo():
    try:
        ...
        session["mask"], session["is_default_mask"] = personalizer.get_mask_details(
            session["nickname"]
        )
        ...
        if opted_in_user:
            ...
            topics_list = gemini.fit_prompt(
                prompt_handler.get_dynamo_highlights(history)
            ).split("#")
            if not history or history == "":
                ...
                if not session["mask"]:
                    icebreaker = gemini.fit_prompt(
                        prompt_handler.get_relevant_icebreaker(
                            session["nickname"], history
                        )
                    )
                else:
                    icebreaker = gemini.fit_prompt(
                        prompt_handler.get_relevant_icebreaker(
                            session["nickname"], history, instructions=session["mask"]
                        )
                    )
                session["tip"] = gemini.fit_prompt(prompt_handler.get_tip(history))
                session["icebreaker"] = icebreaker
            gemini.config_dynamo(
                session["icebreaker"], user, history, instructions=session["mask"]
            )
        else:
            icebreaker = dynamic_web.get_dynamo_icebreaker()
            session["icebreaker"] = icebreaker
            ...
            gemini.config_dynamo(
                session["nickname"], user, history=None, instructions=session["mask"]
            )
        return render_template(
            "dynamo.html",
            talk=session["icebreaker"],
            data=session,
            topics=topics_list,
            error=False,
        )
        ...
        if opted_in_user:
            ...
            if not user_in_chats:
                mongo.db.chats.insert_one(
                    {"nickname": session["nickname"], "history": ""}
                )
            user_in_chats = mongo.db.chats.find_one({"nickname": session["nickname"]})
            previous_history = user_in_chats["history"]
            if previous_history == "" or previous_history[-1] == "~":
                previous_history += "\n[CHAT] " + session["icebreaker"]
            current_history = personalizer.build_history(
                previous_history, str(latest_message), user_message, model="Dynamo"
            )
            updated_history = mongo.db.chats.update_one(
                {"nickname": session["nickname"]},
                {"$set": {"nickname": session["nickname"], "history": current_history}},
            )
...
def create_mask():
    ...
    status = False
    try:
        status = dynamic_web.get_instruction_status(
            gemini.fit_prompt(prompt_handler.get_dynamo_mask(instructions))
        )
    except:
        status = False
    ...
    if status:
        ...
        if existing_user:
            mongo.db.masks.update_one(
                {"nickname": session["nickname"]},
                {"$set": {"default_mask": None, "mask": instructions}},
            )
        else:
            mongo.db.masks.insert_one(
                {
                    "nickname": session["nickname"],
                    "default_mask": None,
                    "mask": instructions,
...
def default_mask(token):
    try:
        ...
        if existing_user:
            mongo.db.masks.update_one(
                {"nickname": session["nickname"]},
                {"$set": {"default_mask": token, "mask": instructions}},
            )
        else:
            mongo.db.masks.insert_one(
                {
    ...
    try:
        ...
        if opted_in_user:
            ...
            if not history or history == "":
                ...
                icebreaker = gemini.fit_prompt(
                    prompt_handler.get_relevant_icebreaker(session["nickname"], history)
                )
                session["tip"] = gemini.fit_prompt(prompt_handler.get_tip(history))
                session["icebreaker"] = icebreaker
        ...
        return render_template(
            "shaman.html", talk=session["icebreaker"], data=session, error=False
        )
    except KeyError:
        return redirect("/")
...
def chat_shaman():
    try:
        ...
        if opted_in_user:
            ...
            updated_history = mongo.db.shaman.update_one(
                {"nickname": session["nickname"]},
                {"$set": {"nickname": session["nickname"], "history": current_history}},
            )
        return jsonify(talk=markdown2.markdown(response.text), error=False)
...
def journal():
    ...
    existing_journals = mongo.db.journals.find(
        {"nickname": session["nickname"]}, {"_id": False}
    )
...
def do_reflection():
    ...
    return jsonify({"status": "success", "questionSet": question_set})
...
def timeline():
    ...
    if (
        existing_user
        and existing_user.get("last_interacted")
        and existing_user.get("last_interacted") == now
    ):
    ...
    if not existing_user:
        mongo.db.timeline.insert_one(
            {"nickname": session["nickname"], "corpus": "", "history": ""}
        )

    if existing_user and (
        existing_user["corpus"] != "" or existing_user["history"] != ""
    ):
...
def update_mood():
    ...
    if existing_user and existing_user["last_interacted"] != date:
        mongo.db.timeline.update_one(
            {"nickname": session["nickname"]},
            {
                "$set": {
                    ...
                    "corpus": personalizer.build_corpus_history(
                        session["nickname"], date, corpus
                    ),
                    "history": personalizer.build_timeline_history(
                        session["nickname"], date, detected_mood
                    ),
                }
            },
        )
        ...
        return jsonify(
            {"status": "error", "flash": "Cannot submit another note today."}
        )
    return jsonify(
        {"status": "success", "flash": "Save successful! Come back again tommorrow!"}
    )
...
def radar():
    ...
    session["rq_id"] = 0
    session["question"] = dynamic_web.get_radar_question(session["rq_id"])
    session["radar_scores"] = []
    session["radar_history"] = ""
...
def radar_response():
    session["rq_id"] += 1
    q_0 = dynamic_web.get_radar_question(0)
    q_t = session["question"]
    user_response = request.form["response"]

    if session["rq_id"] == 1:
        session["radar_history"] += f"Q:{q_0}\nA:{user_response}\n"
    else:
        session["radar_history"] += f"Q:{q_t}\nA:{user_response}\n"

    gemini_response = gemini.chat_radar(session["question"], user_response).text
    response = gemini_response.strip()

    try:
        q_score = int(response)
        session["radar_scores"].append(q_score)
    except ValueError:
        return jsonify({"status": "error"})

    status = 0
    scores = session["radar_scores"]
    verdict, concerns = None, None
    if len(scores) == 17:
        verdict = models.make_radar_verdict_v3(scores)
        concerns = str(gemini.get_radar_concerns(session["radar_history"]))
        session["radar_scores"].append(verdict)
        status = 1

    if status == 0:
        return jsonify(
            question=dynamic_web.get_radar_question(session["rq_id"]), success=True
        )
    elif status == 1:
        return jsonify(
            score=scores,
...
def get_radar_analysis():
    session["rq_id"] = 0
    scores = personalizer.get_scores(session["radar_scores"])
...
def ace():
    ...
    if habits:
        ...
        for habit in habits:
            ...
            if habit.get("score") != 0 and habit.get("check_next") > 0:
                if (
                    habit.get("interacted") == dynamic_web.yesterday()
                    or habit.get("interacted") == dynamic_web.today()
                ):
    ...
    return render_template(
        ...
        completed_habits=session["completed_habits_list"],
        ongoing_habits=session["ongoing_habits_list"],
    )
    ...
    if existing_user:
        ...
        try:
            mongo.db.plans.update_one(
                ...
                {
                    "$set": {
                        "num_tasks": existing_user["num_tasks"] + 1,
                        task_name: task_desc,
                    }
                },
            )
...
def delete_task(token):
    ...
    if existing_user:
        updated_num_tasks = (
            existing_user["num_tasks"] - 1 if existing_user["num_tasks"] > 0 else 0
        )
        mongo.db.plans.update_one(
            {"nickname": session["nickname"]},
            {"$unset": {token: ""}, "$set": {"num_tasks": updated_num_tasks}},
        )
...
def projects():
    ...
    if existing_user:
        ...
        all_projects = mongo.db.projects.count_documents(
            {"nickname": session["nickname"]}
        )
        projects_ = (
            mongo.db.projects.find({"nickname": session["nickname"]}, {"_id": False})
...
def create_project():
    ...
    existing_title_count = mongo.db.projects.count_documents(
        {"nickname": session["nickname"], "project_title": project_title}
    )
...
def new_project(token):
    project_id = token
    project = dict(
        mongo.db.projects.find_one(
            {"nickname": session["nickname"], "project_id": token}, {"_id": False}
        )
    )
    conv = None
    ...
    if project["init"] == True:
        ...
        if not project["context"] or project["context"] == "":
            ...
            if project["active"] != 0:
                history = dynamic_web.get_context(project["context"]).split(
                    "$$$BREAK$$$"
                )[-1]
                ...
                last_catchup_time = project.get(
                    "last_catchup_time", now - timedelta(days=1)
                )
                ...
                elif (not project.get("first_chat_processed", False)) and project.get(
                    "first_chat"
                ) == now.strftime("%d/%m/%Y"):
                    catchup_context = prompt_handler.get_discussion_icebreaker(
                        project["context"]
...
def delete_project(token):
    ...
    if existing_user:
        mongo.db.projects.delete_one(
            {"nickname": session["nickname"], "project_id": token}
        )
    else:
        session["projects"] = False
    ...
    mongo.db.projects.update_one(
        {"nickname": session["nickname"], "project_id": token},
        {
            "$set": {
                ...
                "active": 1,
                "discussions": [
                    {"discussion_title": "Discussion", "discussion_history": ""}
                ],
            }
        },
    )
...
def chat_project_ace(token):
    ...
    if "attachment" in request.files:
        ...
        if (file_extension not in allowed_text_extensions) and (
            file_extension not in allowed_image_extensions
        ):
            return jsonify(
                talk="ERROR: Invalid file type. Ace only serves text, code or image types.",
                catchup=None,
                error=False,
            )
        ...
        if file_extension in allowed_image_extensions:
            ...
            current_history = personalizer.build_project_history(
                session["nickname"],
                project["context"],
                str(latest_message),
                message,
            ...
            mongo.db.projects.update_one(
                {"nickname": session["nickname"], "project_id": token},
                {"$set": {"context": current_history}},
            )
            talk = markdown2.markdown(
                response.text,
                extras=["fenced-code-blocks", "code-friendly", "highlightjs-lang"],
            )
    ...
    current_history = personalizer.build_project_history(
        session["nickname"], project["context"], response.text, message, model="Ace"
    )
    talk = markdown2.markdown(
        response.text,
        extras=["fenced-code-blocks", "code-friendly", "highlightjs-lang"],
    ...
    if not project.get("first_chat", None):
        mongo.db.projects.update_one(
            {"nickname": session["nickname"], "project_id": token},
            {"$set": {"first_chat": datetime.now().strftime("%d/%m/%Y")}},
        )

    mongo.db.projects.update_one(
        {"nickname": session["nickname"], "project_id": token},
        {"$set": {"context": current_history, "last_chat": datetime.now()}},
    )
...
def new_discussion(token):
    query = {"discussions.discussion_title": request.form["title"]}
    ...
    if not document and project["active"] != 1:
        ...
        flash(
            f"Cannot create a new discussion. Please conclude existing ones.", "error"
        )
    return redirect(f"/project/{token}")
...
def conclude_discussion(token, title):
    ...
    query = {"discussions.discussion_title": title}
    document = mongo.db.projects.find_one(query, {"_id": 1})
    if not document:
        flash(f"A discussion titled '{title}' does not exist.", "error")
        return redirect(f"/project/{token}")

    latest_discussion = dynamic_web.get_context(
        str(project["context"]).split("$$$BREAK$$$")[-1]
    )
    marked_history = markdown2.markdown(
        latest_discussion,
        extras=["fenced-code-blocks", "code-friendly", "highlightjs-lang"],
...
def delete_discussion(token):
    ...
    new_context = "$$$BREAK$$$".join(
        contexts[:discussion_index] + contexts[discussion_index + 1 :]
    )
    ...
    mongo.db.projects.update_one(
        {"nickname": session["nickname"], "project_id": token},
        {
            ...
            "$pull": {"discussions": {"discussion_title": request.form["title"]}},
        },
    )
...
def edit_discussion(token):
    ...
    discussion_exists = any(
        discussion["discussion_title"] == prev_title
    ...
    mongo.db.projects.update_one(
        ...
        {"$set": {"discussions.$.discussion_title": new_title}},
    )
    return redirect(f"/project/{token}")
...
def create_habit():
    ...
    if not exists:
        mongo.db.habits.insert_one(
            {"nickname": session["nickname"], "tracked_habits": []}
        )
        exists = True
    ...
    query = {"nickname": session["nickname"], "tracked_habits.habit_name": habit_name}
...
def update_habit(nickname, habit):
    ...
    if habit_doc:
        ...
        if habit_data["score"] != 0 and habit_data["check_next"] > 0:
            if not (
                habit_data.get("interacted") == dynamic_web.yesterday()
                or habit_data.get("interacted") == dynamic_web.today()
            ):
                return jsonify(success=False, missed=True)

        update = {
            "$inc": {"tracked_habits.$.score": 1, "tracked_habits.$.check_next": -1},
            "$set": {"tracked_habits.$.interacted": dynamic_web.today()},
        }
        ...
        if result.modified_count > 0:
            return jsonify(
                success=True, max_score=habit_data["max_score"], score=new_score
            )
        else:
...
def seeker_intro():
    ...
    mongo.db.seeker.update_one(
        {"nickname": session["nickname"], "survey": "False"},
        {
            "$set": {
                "survey": "True",
                "topics": topics,
...
def seeker():
    ...
    mongo.db.users.update_one(
        {"nickname": session["nickname"]}, {"$inc": {"seeker": 1}}
    )
    ...
    try:
        ...
        if opted_in_user:
            ...
            if not history or history == "":
                ...
                icebreaker = gemini.fit_prompt(
                    prompts.prompt_corpus().get_relevant_icebreaker(
                        session["nickname"], history
                    )
                )
                session["tip"] = gemini.fit_prompt(
                    prompts.prompt_corpus().get_tip(history)
                )
                session["icebreaker"] = icebreaker
        ...
        return render_template(
            "seeker.html",
            talk=session["icebreaker"],
            philosopher_talks=philosopher_talks,
            data=session,
            error=False,
            user_topics=user_topics,
            fact=session.get("fact", ""),
            fact_topic=session.get("fact_topic", ""),
        )
    ...
    try:
        ...
        if opted_in_user:
            ...
            if previous_history == "" or previous_history[-1] == "~":
                previous_history += "\n[CHAT] " + session["icebreaker"]
            current_history = personalizer.build_history(
                previous_history, str(latest_message), user_message, model="Seeker"
            )
            updated_history = mongo.db.seeker.update_one(
                {"nickname": session["nickname"]},
                {"$set": {"nickname": session["nickname"], "history": current_history}},
            )
        return jsonify(talk=markdown2.markdown(response.text), error=False)
...
def philosopher_mask(token):
    try:
        resp = gemini.config_philosopher(
            session["nickname"],
            token,
            icebreaker=session["philosopher_icebreakers"][token],
        )
